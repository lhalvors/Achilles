...
from (select * from @results_database_schema.ACHILLES_results where analysis_id = 2800) ar1
	inner join
	(select * from @results_database_schema.ACHILLES_results where analysis_id = 2801) ar2
	...
	(
		...
		join @cdm_database_schema.sb_fact f on c.concept_id = f.fact_concept_id
